				...
				case "mod":
					switch (first) {
						case "ban":
							...
							sender.sendMessage(prefix + Utils.color("lang.mod.ban").replaceAll("%user%", args[1]));
							return true;
						case "unban":
							dta.getRoom().unbanUser(args[1]);
							sender.sendMessage(prefix + Utils.color("lang.mod.unban").replaceAll("%user%", args[1]));
							return true;
						case "kick":
							dta.getRoom().kickUser(args[1]);
							sender.sendMessage(prefix + Utils.color("lang.mod.kick").replaceAll("%user%", args[1]));
							return true;
						case "skip":
